...
export default function Header() {
  ...
  // Handle alert visibility: only on homepage + not previously dismissed
  useEffect(() => {
    if (!isHomePage) {
      setShowAlert(false);
      return;
  ...
  const handleDismiss = () => {
    setShowAlert(false);
    localStorage.setItem("eventAlertDismissed_2026", "true");
  };
  ...
  return (
    <>
      ...
      {/* Event Reminder Alert - Only on homepage + dismissible permanently */}
      {showAlert && (
